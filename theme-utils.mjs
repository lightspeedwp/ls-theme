 ...
 *
 * Commands:
 *   sync-breakpoints  Sync the Sass breakpoint map from theme.json custom layout break-points.
...
const DARK_JSON_PATH = join( ROOT, 'styles', 'dark.json' );
const BREAKPOINT_SCSS_PATH = join( ROOT, 'src', 'scss', 'abstracts', 'mixins', '_breakpoints-theme.scss' );
const THEME_TOGGLE_CSS_PATH = join( ROOT, 'assets', 'css', 'theme-toggle.css' );
...
/**
 * Sync the generated Sass breakpoint map from theme.json.
 */
async function syncBreakpoints() {
	heading( 'Syncing Sass breakpoints from theme.json…' );

	const themeJSON = readJSON( THEME_JSON_PATH );
	const breakpoints = themeJSON?.settings?.custom?.layout?.[ 'break-points' ];

	if ( ! breakpoints || typeof breakpoints !== 'object' || Array.isArray( breakpoints ) ) {
		error( 'theme.json is missing settings.custom.layout.break-points.' );
		process.exit( 1 );
	}
	...
	if ( 0 === entries.length ) {
		error( 'theme.json settings.custom.layout.break-points is empty.' );
		process.exit( 1 );
	}

	for ( const [ name, value ] of entries ) {
		if ( ! /^[a-z0-9-]+$/u.test( name ) ) {
			error( `Invalid breakpoint name '${ name }' in theme.json.` );
			process.exit( 1 );
		}

		if ( 'string' !== typeof value || '' === value.trim() ) {
			error( `Breakpoint '${ name }' must be a non-empty string value in theme.json.` );
			process.exit( 1 );
		}
	}

	const scss = [
		'// Auto-generated from theme.json by `node theme-utils.mjs sync-breakpoints`.',
		...
		...entries.map( ( [ name, value ], index ) => {
			...
			return `${ keyValuePair }${ isLastEntry ? '' : ',' }`;
		} ),
		');',
		'',
	].join( '\n' );

	const existing = existsSync( BREAKPOINT_SCSS_PATH ) ? readFileSync( BREAKPOINT_SCSS_PATH, 'utf8' ) : null;

	if ( existing === scss ) {
		success( `${ relative( ROOT, BREAKPOINT_SCSS_PATH ) } is already up to date.` );
		return;
	}

	writeFileSync( BREAKPOINT_SCSS_PATH, scss, 'utf8' );
	success( `Updated ${ relative( ROOT, BREAKPOINT_SCSS_PATH ) } from theme.json break-points.` );
}

/**
 * Recursively flatten a nested settings.custom.* object into
 * { 'color--text--default': 'var:preset|color|base', ... } entries, joined with '--'
 * to match WordPress's own custom-property naming convention.
 *
 * @param {object} obj          Object to flatten.
 * @param {string[]} pathParts  Path segments accumulated so far.
 * @param {object} out          Accumulator.
 */
function flattenCustomTree( obj, pathParts, out ) {
	for ( const [ key, value ] of Object.entries( obj ) ) {
		const parts = [ ...pathParts, key ];
		if ( value && 'object' === typeof value && ! Array.isArray( value ) ) {
			flattenCustomTree( value, parts, out );
		} else {
...
/**
 * Convert a theme.json `var:preset|type|slug` reference into a real CSS `var(...)` reference.
 * Values that aren't preset references (e.g. literal shadow formulas) are returned unchanged —
 * they already use `var(--wp--custom--...)` internally, which resolves correctly once those
 * referenced custom properties are also re-pointed by the same override block.
 *
 * @param {string} value Raw theme.json value.
 * @returns {string} CSS-ready value.
 */
function presetRefToCssVar( value ) {
	const match = /^var:preset\|([a-z-]+)\|([a-z0-9-]+)$/iu.exec( value );
	if ( ! match ) {
		return value;
	}
...
/**
 * Generate assets/css/theme-toggle.css: two `:root[data-theme="…"]` blocks that re-point every
 * settings.custom.color / settings.custom.shadow token to the light (theme.json) or dark
 * (styles/dark.json) preset it should resolve to. Powers the header's client-side light/dark
 * toggle — WordPress's own light/dark "mode" is a full theme.json style variation picked by an
 * admin in the Site Editor, not something a site visitor can switch at runtime, so this file is
 * what makes an actual visitor-facing toggle possible without hand-duplicating every token.
 */
async function generateThemeToggleCss() {
	heading( 'Generating theme-toggle CSS from theme.json + styles/dark.json…' );

	const themeJSON = readJSON( THEME_JSON_PATH );
	const darkJSON = readJSON( DARK_JSON_PATH );

	if ( ! themeJSON || ! darkJSON ) {
		error( 'Could not read theme.json or styles/dark.json.' );
		process.exit( 1 );
	}

	const lightFlat = {};
	flattenCustomTree( themeJSON?.settings?.custom?.color || {}, [ 'color' ], lightFlat );
	flattenCustomTree( themeJSON?.settings?.custom?.shadow || {}, [ 'shadow' ], lightFlat );

	const darkFlat = {};
	flattenCustomTree( darkJSON?.settings?.custom?.color || {}, [ 'color' ], darkFlat );
	flattenCustomTree( darkJSON?.settings?.custom?.shadow || {}, [ 'shadow' ], darkFlat );
	...
	const toBlock = ( flat ) =>
		Object.entries( flat )
			.map( ( [ path, value ] ) => `  --wp--custom--${ path }: ${ presetRefToCssVar( value ) };` )
			.join( '\n' );

	const css = [
		'/* Auto-generated from theme.json + styles/dark.json by `node theme-utils.mjs generate-theme-toggle`. */',
		'/* Do not edit this file by hand — edit the source JSON and regenerate. */',
		'',
	...
	const existing = existsSync( THEME_TOGGLE_CSS_PATH ) ? readFileSync( THEME_TOGGLE_CSS_PATH, 'utf8' ) : null;

	if ( existing === css ) {
		success( `${ relative( ROOT, THEME_TOGGLE_CSS_PATH ) } is already up to date.` );
		return;
	}

	writeFileSync( THEME_TOGGLE_CSS_PATH, css, 'utf8' );
	success( `Generated ${ relative( ROOT, THEME_TOGGLE_CSS_PATH ) } from theme.json + styles/dark.json.` );
}
...
Commands:
	${ CYAN }sync-breakpoints${ RESET }
		Sync the Sass breakpoint map from theme.json custom layout break-points.

  ${ CYAN }generate-theme-toggle${ RESET }
    Generate assets/css/theme-toggle.css from theme.json + styles/dark.json, powering the
    header's client-side light/dark toggle.
...
switch ( command ) {
	case 'sync-breakpoints':
		await syncBreakpoints();
		break;
	case 'generate-theme-toggle':
		await generateThemeToggleCss();
		break;
