...
are flat, confirmed by inspecting dev-site page parent/child relationships. However, the
check did surface **4 pages that existed on dev with no Linear issue**: Accessibility
Commitment (About), and Publishing, Design Systems, and LSX (Solutions). These have been
created as LS-4193–LS-4196 and are included below. This batch is **19 pages**, not the
originally-assumed 15.
...
1. **Given** a visitor on the Free Consultation page, **When** they complete and submit the
   form, **Then** the submission is captured and the visitor sees confirmation (routes to the
   Contact thank-you page or an equivalent confirmation state).
...
A visitor wants to contact LightSpeed directly, or needs to read a legal policy (Privacy
Policy, Terms & Conditions, Policies & Principles).

**Why this priority**: Contact is a secondary conversion path (after Free Consultation); the
legal pages are compliance requirements, not conversion-critical, but bundled here because
they share the same low-bespoke-effort Default Page template and the same Linear issue
(LS-1605).

**Independent Test**: Can be fully tested by visiting each of the four pages directly and
...
1. **Given** a visitor on the Contact page, **When** they submit the contact form, **Then**
   the submission is captured and the visitor sees confirmation.
2. **Given** a visitor on the Privacy Policy, Terms & Conditions, or Policies & Principles
   page, **When** they read it, **Then** the approved legal content is displayed correctly
   (content itself is already finalised and approved per LS-1224 — this batch wires it in,
...
A visitor who has just submitted the Contact form (or, per User Story 1, the Free
Consultation form) reaches a thank-you confirmation page.
...
- **FR-004**: The Privacy Policy, Terms & Conditions, and Policies & Principles pages MUST
  display their already-approved legal content (LS-1224) correctly — this batch wires
  existing approved content into pages, it does not author new legal content.
- **FR-005**: The Contact thank-you page MUST display a clear confirmation message
  independent of how the visitor arrived at it.
...
- **FR-009**: Every page in this batch (19 total) MUST meet the constitution's Accessibility
  Baseline (WCAG 2.1 AA).
- **FR-010**: Every page in this batch MUST carry working SEO metadata (title, description).
- **FR-011**: Every page in this batch MUST be verified in both light and dark mode and
  across mobile, tablet, and desktop breakpoints before being considered complete.
- **FR-012**: Implementation of any page in this batch MUST NOT proceed on visual/layout
...
- **Conversion Page**: Free Consultation or Contact — a page whose primary purpose is
  capturing a form submission and routing to a thank-you confirmation.
...
- **Family Hub**: About or Solutions — a page linking to a fixed set of related subpages
  within its family.
- **Legal Page**: Privacy Policy, Terms & Conditions, or Policies & Principles — static
  content pages with already-approved copy, no dynamic behavior.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: A visitor can complete a Free Consultation submission and reach a confirmation
  state without encountering a broken step.
- **SC-002**: A visitor can complete a Contact submission and reach a confirmation state
  without encountering a broken step.
- **SC-003**: A visitor can identify and reach any of the 5 About-family pages or 7
  Solutions-family pages within one visit to the respective hub.
- **SC-004**: Every page in this batch (19 total) passes an accessibility spot-check (WCAG
...
- Figma frames for each of the 19 pages will be supplied by the site owner individually, at
  the point implementation of that specific page begins.
- Legal page content (Privacy Policy, Terms & Conditions, Policies & Principles) is already
  finalised and approved (LS-1224) — this batch is a wiring/build task, not a content-drafting
  task.
...
- The Contact form and Free Consultation form both route to a thank-you confirmation state;
  whether they share the single tracked Contact thank-you page (LS-2595) or need distinct
  confirmation pages is an implementation-time decision, not resolved here — the existing
  dev site currently has a separate `/free-consultation/thank-you/` page, which weighs toward
  each form having its own thank-you page rather than sharing one; this is flagged for
  `/speckit-plan` to resolve, not assumed silently.
