...
- Fixed the header light/dark toggle button (`.is-style-header-icon-button`) rendering with no circular button shell — only its `:hover`/`:focus-within` treatment existed; added the base border/background/sizing CSS documented in `styles/blocks/groups/header-icon-button.json`, plus a `.ls-theme-toggle` reset so the inner `<button>` drops native browser chrome (LS-1618).
- Fixed the header search field and icon toggle rendering incorrectly in the Site Editor (search input shown expanded by default; toggle button unstyled) by adding `assets/css/animations.css` via `add_editor_style()` in `functions.php`, so it reliably reaches the Site Editor's iframed canvas rather than only the outer wp-admin document (LS-1618).
...
- Added a `site-logo--dark` / `site-logo--light` CSS swap for the header logo (`src/scss/animations/_header-motion.scss`), matching the existing sun/moon icon-swap convention. Intentionally not baked into `patterns/header.php` — a logo image block is a Media Library upload, so its attachment ID and URL are specific to each site's database; apply the two classes to the light/dark logo blocks per-environment in the Site Editor (LS-1618).
