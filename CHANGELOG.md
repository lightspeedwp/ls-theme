...
- Fixed a render-order bug in `inc/portfolio-card-colors.php`: the WooCommerce card-colour swap now runs on `render_block_data` (before WordPress decides which block-style variation to generate CSS for) instead of `render_block` (which only edited the rendered HTML string after that decision was already made), so WooCommerce-tagged Portfolio cards no longer silently lose their intended styling.
